...
from logical full path into the `index_name` and vice versa. For
...
case, both the actual indexed name to match against the index
will be resolved, and an automatic query filter will be added to only match
`person` types.
